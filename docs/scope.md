...
- An authenticated participant with an existing `cancelled` Event Registration may regain active participation through that same Registration only when the Event exists, is `published`, and organizer-controlled new Registration availability is `open`; an ineligible attempt leaves the Registration `cancelled`.
- Successful re-participation preserves Registration identity and registrant/target uniqueness and changes the same durable Registration from `cancelled` to `active`. It never creates a second Registration for the occupied pair; an already-`active` same-pair Registration likewise creates no duplicate durable state.
- Re-participation does not automatically create, delete, promote, complete, or otherwise mutate Waitlist participation.
...
- Registration owns a domain-neutral registrant-to-target relation, uniqueness, the `active -> cancelled` and `cancelled -> active` lifecycle transitions, durable retrieval, and idempotent cancellation.
- Both lifecycle transitions preserve Registration identity and the complete registrant-target uniqueness relation; the pair remains occupied throughout the lifecycle.
...
- Event-Registration owns Event-specific cross-capability orchestration, including deciding participant Registration creation and same-Registration re-participation eligibility from Event-owned lifecycle and Registration-availability state, mapping the authenticated participant to the registrant reference, and presenting Registration state targeting an owned Event to its organizer through public module APIs.
